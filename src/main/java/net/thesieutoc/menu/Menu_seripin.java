...
public class Menu_seripin implements Listener {
    ...
    public static void seri(Player p) {
        (new AnvilGUI.Builder()).onClick((slot, stateSnapshot) -> {
            ...
            JsonObject json = (JsonObject)m.REQUESTS.getOrDefault(p.getName(), new JsonObject());
            json.addProperty("seri", reply);
            m.REQUESTS.put(p.getName(), json);
            return AnvilGUI.Response.close();
        }).onClose((player) -> Bukkit.getScheduler().runTask(m, () -> pin(p))).title(get("Seri").getItemMeta().getDisplayName()).itemLeft(get("Seri")).text(ChatColor.stripColor(get("Seri").getItemMeta().getDisplayName())).plugin(m).open(p);
    }
    ...
    public static void pin(Player p) {
        (new AnvilGUI.Builder()).onClick((slot, stateSnapshot) -> {
            ...
            return AnvilGUI.Response.close();
        }).title(get("Pin").getItemMeta().getDisplayName()).itemLeft(get("Pin")).text(ChatColor.stripColor(get("Pin").getItemMeta().getDisplayName())).plugin(m).open(p);
    }
}
