...
public class Flatfile {
    public void writeLog(Player p, JsonObject json) {
        Date now = new Date();
        String msg = p.getName() + " | " + json.get("seri").getAsString() + " | " + json.get("pin").getAsString() + " | " + json.get("cardprice").getAsString() + " | " + json.get("cardtype").getAsString() + " | " + json.get("msg").getAsString();
        SimpleDateFormat df = new SimpleDateFormat("HH:mm dd/MM/yyyy");
        File log = new File(Thesieutoc.getInstance().getDataFolder(), "log" + (json.get("msg").getAsString().equals("thanh cong") ? "_success" : "_failed") + ".txt");
        if (!log.exists()) {
